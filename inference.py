...
def inference(model_name, weight_file, image_size, image_list, data_dir, label_dir, return_results=True,
              save_dir='val_result',
              label_suffix='.png',
              data_suffix='.jpg'):
    ...
    for img_num in image_list:
        ...
        image = np.lib.pad(image, ((pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2), (0, 0)),
        ...
        # result_img.palette = [(0, 0, 0), (128, 0, 0), (0, 128, 0), (128, 128, 0), (0, 0, 128), (128, 0, 128),
        ...
        result_img = result_img.resize(img_size, resample=Image.BILINEAR)
        width, height = result_img.size
        for x in range(0, width):
            for y in range(0, height):
                current_color = result_img.getpixel((x, y))
                result_img.putpixel((x, y), (color_dict.get(current_color)))
...
if __name__ == '__main__':
    ...
    weight_file = 'checkpoint_weights4_miou0.18.hdf5'
    image_size = (512, 512)
    data_dir = os.path.expanduser('images')
    label_dir = os.path.expanduser('annotations_trainval')

    image_list = os.listdir('val')  # '2007_000491'
